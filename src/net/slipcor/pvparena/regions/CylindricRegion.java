...
public class CylindricRegion extends ArenaRegionShape {
	...
	public void showBorder(final Player player) {
		...
		PABlockLocation lowercenter = new PABlockLocation(getCenter()
				.toLocation());
		PABlockLocation center = new PABlockLocation(lowercenter.toLocation());
		PABlockLocation uppercenter = new PABlockLocation(
		...
		for (int x = 0; x <= Math.ceil(radius + 1 / 2); x++) {
			...
			border.add((new Location(w, center.getX() + x, center.getY(),
			...
			border.add((new Location(w, lowercenter.getX() + x, lowercenter
			...
			border.add((new Location(w, uppercenter.getX() + x, uppercenter
	...
	public boolean tooFarAway(int joinRange, Location location) {
		...
		PABlockLocation reach = (new PABlockLocation(location)).pointTo(cLoc,
				(double) joinRange);
