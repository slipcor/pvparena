...
public class LanguageManager {
	...
	public LanguageManager() {
		...
		config.addDefault("log.nospout","Spout not found, you are missing some features ;)");
		config.addDefault("log.enabled","enabled (version %1%)");
		config.addDefault("log.disabled","disabled (version %1%)");
		config.addDefault("log.noperms","Permissions plugin not found, defaulting to OP.");
		...
		config.addDefault("lang.usepatoexit","Please use '/pa leave' to exit the fight!");
		config.addDefault("lang.pos1","First position set.");
		config.addDefault("lang.pos2","Second position set.");
		config.addDefault("lang.classperms","You don't have permission for that class.");
		config.addDefault("lang.toomanyplayers","There are too many of this class, pick another class.");
		config.addDefault("lang.notready","Not everyone has picked a class!");
		config.addDefault("lang.waitequal","Waiting for the teams to have equal player number!");
		...
		config.addDefault("lang.lostlife","%1% has lost a life! %2% remaining.");
		config.addDefault("lang.onlyplayers","Only players may access this command!");
		config.addDefault("lang.arenadisabled","Arena disabled, please try again later!");
		config.addDefault("lang.arenanotsetup","All waypoints must be set up first. %1%");
		config.addDefault("lang.permjoin","You don't have permission to join the arena!");
		...
		config.addDefault("lang.fightinprogress","A fight is already in progress!");
		...
		config.addDefault("lang.specwelcome","Welcome to the spectator's area! /pa bet [name] [amount] to bet on team or player");
		...
		config.addDefault("lang.forcestop","You have forced the fight to stop.");
		config.addDefault("lang.nofight","There is no fight in progress.");
		config.addDefault("lang.invalidcmd","Invalid command (%1%)");
		config.addDefault("lang.betnotyours","Cannot place bets on your own match!");
		config.addDefault("lang.betoptions","You can only bet on team name or arena player!");
		...
		config.addDefault("lang.invfull","Your inventory was full. You did not receive all rewards!");
		...
		config.addDefault("lang.regionalreadybeingset","A region is already being created: %1%");
		config.addDefault("lang.regionnotbeingset","A region is not being created!");
		...
		config.addDefault("lang.selectteam","You must select a team to join! /pa [arenaname] [team]");
		config.addDefault("lang.notselectteam","You cannot select a team to join! /pa [arenaname]");
		config.addDefault("lang.joinrange","You are too far away to join this arena!");
		config.addDefault("lang.playerpowerup","Player %1% receives powerup %2%!");
		config.addDefault("lang.serverpowerup","Powerup %1% deployed!");
		config.addDefault("lang.flaggrab","Player %1% grabbed the flag of team %2%!");
		config.addDefault("lang.flaghome","Player %1% brought home the flag of team %2%!");
		config.addDefault("lang.flagsave","Player %1% dropped the flag of team %2%!");
		config.addDefault("lang.teamhaswon","%1% are the Champions!");
		config.addDefault("lang.playerhaswon","%1% is the Champion!");
		config.addDefault("lang.notready","Not everyone has picked a class!");
		
		config.addDefault("lang.checkregionerror", "Waiting for a running arena to finish!");
		...
		lang = (Map<String, Object>) config.getConfigurationSection("lang").getValues(true);
		log = (Map<String, Object>) config.getConfigurationSection("log").getValues(true);;
